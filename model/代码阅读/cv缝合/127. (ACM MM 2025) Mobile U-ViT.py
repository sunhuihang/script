...
class ConvUtr(nn.Module):
    def __init__(self, ch_in, ch_out, depth=1, kernel=3):
        ...
        self.block = nn.Sequential(
            *[nn.Sequential(
                Residual(nn.Sequential(
                    nn.Conv2d(ch_in, ch_in, kernel_size=(kernel, kernel), groups=ch_in, padding=(kernel // 2, kernel // 2)),                                                                                                                     # 微信公众号:CV缝合救星
                    nn.GELU(),
                    nn.BatchNorm2d(ch_in)
                )),
                Residual(nn.Sequential(
                    nn.Conv2d(ch_in, ch_in * 4, kernel_size=(1, 1)),                                                                                                                     # 微信公众号:CV缝合救星
                    nn.GELU(),
                    nn.BatchNorm2d(ch_in * 4),
                    nn.Conv2d(ch_in * 4, ch_in, kernel_size=(1, 1)),                                                                                                                     # 微信公众号:CV缝合救星
                    nn.GELU(),
                    nn.BatchNorm2d(ch_in)
                )),
        ...
        self.up = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=3, stride=1, padding=1, bias=True),                                                                                                                     # 微信公众号:CV缝合救星
...
class Embeddings(nn.Module):
    def __init__(self, inch=3, dims=[8, 16, 32], depths=[1, 1, 3], kernels=[3, 3, 7]):                                                                                                                     # 微信公众号:CV缝合救星
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):                                                                                                                     # 微信公众号:CV缝合救星
        super().__init__()
...
class GlobalSparseAttn(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1.):                                                                                                                     # 微信公众号:CV缝合救星
        super().__init__()
        ...
        if self.sr > 1:
            ...
            self.LocalProp = nn.ConvTranspose2d(dim, dim, kernel_size, stride=sr_ratio, groups=dim)                                                                                                                     # 微信公众号:CV缝合救星
...
class LGLBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.SelfAttn = SelfAttn(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, drop_path, act_layer,                                                                                                                     # 微信公众号:CV缝合救星
                                 norm_layer, sr_ratio)
...
class conv_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch_out, ch_out, kernel_size=3, stride=1, padding=1, bias=True),                                                                                                                     # 微信公众号:CV缝合救星
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
    ...
    def __init__(self,
        ...
        self.lklgl_bottleneck = nn.ModuleList([
            LGLBlock(
                ...
                drop=0., attn_drop=0., drop_path=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratio=2)                                                                                                                     # 微信公众号:CV缝合救星
            for _ in range(depths[3])])

        self.transformer_bottleneck = nn.ModuleList([
            LGLBlock(
                dim=dims[3], num_heads=embed_dim // 64, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=0., attn_drop=0., drop_path=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratio=1)                                                                                                                     # 微信公众号:CV缝合救星
...
def mobileuvit(inch=3, dims=[16, 32, 64, 128], depths=[1, 1, 3, 3, 3], kernels=[3, 3, 7], embed_dim=256, out_channel=1):
    return MobileUViT(inch=inch, dims=dims, depths=depths, kernels=kernels, embed_dim=embed_dim, out_channel=out_channel)                                                                                                                     # 微信公众号:CV缝合救星


def mobileuvit_l(inch=3, dims=[32, 64, 128, 256], depths=[1, 1, 3, 3, 4], kernels=[3, 3, 7], embed_dim=512, out_channel=1):                                                                                                                     # 微信公众号:CV缝合救星
...
if __name__ == "__main__":
    ...
    # 初始化
    model = mobileuvit(inch=3, dims=[16, 32, 64, 128], depths=[1, 1, 3, 3, 3], kernels=[3, 3, 7], embed_dim=256, out_channel=1)                                                                                                                     # 微信公众号:CV缝合救星
