...
class ChannelAttention(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 添加全局平均池化层
    ...
    def forward(self, x):
        # 打印x的尺寸，查看问题所在
        print(f"x.size() in ChannelAttention: {x.size()}")

        b, c, h, w = x.size()

        # 对每个通道进行全局平均池化
        y = self.avg_pool(x).view(b, c)  # 将高宽压缩为1
...
class LSA(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, _, _ = sq_x.size()
        y = self.avg_pool(sq_x).view(b, c)  # N*C, 16, H/4, W/4 to N*C, 16, 1, 1
        ...
        out = x * ex_y  # Apply the attention to the input
