...
class GRSAWrapper(nn.Module):
    def __init__(self, dim, window_size=(8, 8), num_heads=8, qkv_bias=True, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.window_size = window_size
        self.dim = dim
        self.inner = GRSA(dim, window_size, num_heads, qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 4, 3, 5, 1).contiguous()  # [B, Hp//Wh, Wp//Ww, Wh, Ww, C]
        x = x.view(-1, Wh * Ww, C)  # [num_windows*B, Wh*Ww, C]

        # apply GRSA
        x = self.inner(x)  # [B*n_win, N, C]

        # reshape back
        x = x.view(B, Hp // Wh, Wp // Ww, Wh, Ww, C)
        x = x.permute(0, 5, 1, 3, 2, 4).contiguous()  # [B, C, Hp//Wh, Wh, Wp//Ww, Ww]
        x = x.view(B, C, Hp, Wp)
...
if __name__ == "__main__":
    B, C, H, W = 1, 96, 64, 64  # 输入尺寸
    window_size = (8, 8)

    x = torch.randn(B, C, H, W).cuda()
    model = GRSAWrapper(dim=C, window_size=window_size, num_heads=8).cuda()
    print(model)
    print("哔哩哔哩：CV缝合救星")
    out = model(x)
    print("输入形状:", x.shape)
