...
"""
CV缝合救星魔改创新：ResidualGatedFFT
说明：可学习的残差连接：在 FFTNetBlock 类的 __init__ 方法中添加了一个可学习的参数
 self.residual_weight，初始值为 1。在 forward 方法中，将处理后的输出 x_out 与原
 始输入 x 进行残差连接，并且乘以可学习的权重 self.residual_weight。这样模型可以在
 训练过程中自动学习残差连接的权重，以更好地平衡原始输入信息和处理后的特征信息。这种方
 式可以让模型在不同的任务和数据集上更灵活地利用残差结构，提高模型的性能和稳定性。
"""
...
class FFTNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 添加可学习的残差连接
        x_out = x_out + self.residual_weight * x
        return x_out
