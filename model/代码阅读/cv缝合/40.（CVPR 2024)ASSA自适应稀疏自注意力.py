...
'''
自适应稀疏自注意力模块（ASSA）：用于图像恢复的自适应稀疏自注意力模块（CVPR 2024）
即插即用模块：ASSA（可应用于多种图像恢复任务，提升特征聚合效果）（替身添花模块）
一、背景
在图像恢复任务中，基于 Transformer 的方法虽能建模长距离依赖，但存在计算密集和特征冗余问题。
标准Transformer的密集注意力关系会引入无关区域的噪声交互，且特征图中的冗余信息会阻碍模型关
注有效特征。现有的一些方法如 Top - K 选择或在超像素空间进行自注意力计算虽试图解决，但仍面
临挑战，如参数K对特定任务敏感，超像素空间计算仍涉及所有令牌可能引入不需要的交互。

二、ASSA 模块原理
1. 输入特征：输入的归一化特征图，其具有一定的分辨率和通道数。
2. 模块组成与处理
A. 稀疏自注意力分支（SSA）：为过滤掉低查询 - 键匹配分数特征的负面影响，引入基于平方 ReLU
的自注意力。先将特征图划分为非重叠窗口，从窗口生成查询、键和值矩阵，计算时通过特定公式去除
负分数相似性，传播有用信息，但会触发信息损失。
B. 密集自注意力分支（DSA）：采用 softmax 层，考虑所有查询 - 键对获取注意力分数，但因并非
所有查询令牌都与键密切相关，利用所有相似性对清晰图像重建无效，且会引入无关区域的噪声交互。
C. 自适应融合：提出两分支自注意力机制，通过自适应权重融合两分支，使模型能控制输入令牌稀疏度，
平衡过滤噪声交互和利用有效信息。模型先对两分支同等对待以确保信息充足，随着层数加深更关注稀疏
分支以更好聚合特征，学习到的权重可让模型适应两分支影响。
3. 输出：经过 ASSA 处理后的特征，用于后续图像恢复操作。

三、适用任务
1. 主要适用于图像恢复任务，如雨水条纹去除、真实雾霾去除和雨滴去除等任务。
2. 作为替身添花模块适用于所有CV任务。
'''
