...
即插即用模块：Bi-Level Routing Attention(BRA) 替身模块
一、背景
研究背景与挑战：Transformer 在计算机视觉领域取得了显著进展，但原始的注意力机制在计算效率上存在问题，
因其计算复杂度与输入的空间分辨率呈二次方关系，导致在处理高分辨率图像时计算成本高昂。此外，现有一些改
进方法虽尝试引入稀疏性来降低计算量，但存在局限性，如使用手工设计的固定稀疏模式或在所有查询中共享采样
的键值对，无法根据查询的内容动态适应。

本文提出一种名为 BiFormer 的新型视觉 Transformer，其核心是双层路由注意力BRA机制，能以动态、查询
感知的方式实现高效计算分配，通过在粗粒度区域级别过滤无关键值对，在剩余候选区域应用细粒度注意力，有效
解决了传统注意力机制的计算瓶颈问题。

二、BRA原理详细解读
1. 区域划分与输入投影：将输入特征图划分为个非重叠区域，通过线性投影得到查询、键、值张量。
2. 区域到区域路由（基于有向图）：通过区域平均池化得到区域级查询和键，计算区域间亲和度图的邻接矩阵，
对其进行修剪，保留每个区域的前个连接，得到路由索引矩阵。
3. 令牌到令牌注意力：根据路由索引矩阵收集键值对，在收集后的键值对上应用注意力机制，并引入局部上下文
增强项（LCE），最后将输出恢复到原始特征图形状。
