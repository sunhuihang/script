...
# B站：CV缝合救星
"""
67. D-Net: Reciprocal Attention Mixing Transformer for Lightweight Image Restoration (CVPR 2024 Workshop)
即插即用模块*B站CV缝合救星*：D-RAMiT (互注意力混合ViT) （替身强化模块）

一、背景
在图像恢复领域，现有方法存在参数过多以及无法兼顾局部和全局特征的问题。基于 Transformer 的方法，有的聚焦局部特征导致感受野
受限，有的虽关注全局特征但存在参数缺陷，且计算成本高，难以应用于实际场景。为解决这些问题，提出 RAMiT 网络，旨在实现轻量级且
高效的图像恢复。

二、D-RAMiT 模块介绍
（一）整体设计
D-RAMiT 模块的设计目的*B站CV缝合救星*是解决图像恢复任务中难以同时捕捉局部和全局特征的问题。通过创新的双向二维自注意力机制，
并行计算空间和通道维度的自注意力，并将其融合，从而提升模型对图像特征的捕捉能力，增强图像恢复效果。
（二）核心组件与操作
1. 并行自注意力计算：同时运行空间自注意力（SPSA）和通道自注意力（CHSA）机制。在计算时，为 SPSA 和 CHSA 分配不同数量的多头
，分别计算各自的注意力结果。SPSA 关注局部区域的特征，CHSA 则侧重于捕捉全局特征，两者并行工作，全面获取图像信息。
2. 注意力融合与增强：使用 MobiVari 模块对 SPSA 和 CHSA 的结果进行融合。MobiVari 基于 MobileNet V2 进行改进，通过调整
激活函数、残差连接和卷积方式，不仅将两种注意力结果混合，还进一步增强了局部特征，提升了模型对图像细节的捕捉能力。
3. 双向辅助机制：D-RAMiT 引入双向辅助策略，在计算当前块的 SPSA 时，将前一块 CHSA 的输出与当前块 SPSA 的价值向量进行元素相
乘；计算 CHSA 时则反之。这一操作*B站CV缝合救星*让两个自注意力机制相互补充，弥补各自的缺陷，提升整体性能。不过，每个阶段的首
个 D-RAMiT 块由于无前一块相同分辨率的特征，不进行此操作。

三、微观设计考量
D-RAMiT 模块优势显著。其并行计算和融合空间与通道自注意力的方式，打破了传统方法仅关注局部或全局单一特性的局限，有效扩大了感受野。
双向辅助机制增强了特征之间的交互，进一步提升了特*B站CV缝合救星*征捕捉能力。并且，D-RAMiT 在计算效率上表现出色，相较于纯 SPSA
模块，其时间复杂度更低，在多个图像恢复任务中，以更少的计算量和参数数量取得了更优的结果。

四、适用任务
D-RAMiT 主要应用于多种轻量级图像恢复任务，如超分辨率、低光增强、去雨、彩色去噪和灰度去噪。在这些任务中，D-RAMiT 通过有效捕捉局
部和全局特征，提升了恢复图像的质量。在超分辨率任务中，能更好*B站CV缝合救星*地重建图像细节；在去噪任务里，对噪声的去除效果显著。
实验表明，包含 D-RAMiT 模块的 RAMiT 网络在多个任务上超越了其他先进模型，充分证明了 D-RAMiT 在图像恢复任务中的有效性和实用性。
"""
