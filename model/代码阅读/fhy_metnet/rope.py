...
def apply_rotary_emb(
    ...
    if use_real:
        ...
        # used for lumina head first
        x_rotated = torch.view_as_complex(x.float().reshape(*x.shape[:-1], -1, 2))
        while len(freqs_cis.shape) < len(x.shape):
...
if __name__ == '__main__':
    x = torch.rand(1, 8, 32, 256).cuda()
    fis = get_1d_rotary_pos_embed(256, 32).cuda()
    m = apply_rotary_emb(x, fis, use_real=False)
    print(m.shape)
