...
rtm_option={rtm_option},
only_sea_rad=false,
use_varbc=.true.
crtm_coef_path='{crtm_coef_path}'
...
nio_groups = 1,
/ 
""".format(ob_format=env.ob_format,rtm_option=rtm_option,
           crtm_coef_path=crtm_coef_path,analysis_date=analysis_date,
...
#链接VARBC偏差订正文件
if os.path.exists(f'{VARBC_DIR}/{hh}/VARBC.out_{rtm_type}'):
    os.system(f'cp {VARBC_DIR}/{hh}/VARBC.out_{rtm_type} VARBC.in')
else:
...
gfs_prebufr = f'{GDAS_DIR}/{ccyy}.{mm}/{ccyy}{mm}{dd}{hh}/gfs.t{hh}z.prepbufr.nr'
gdas_prebufr = f'{GDAS_DIR}/{ccyy}.{mm}/{ccyy}{mm}{dd}{hh}/gdas.t{hh}z.prepbufr.nr'
if bufr_type == 'gfs':    
    os.system(f'ln -sf {GDAS_DIR}/{ccyy}.{mm}/{ccyy}{mm}{dd}{hh}/{bufr_type}.t{hh}z.prepbufr.nr ob.bufr')
else:
    if os.path.exists(gdas_prebufr):
        os.system(f'ln -sf {gdas_prebufr} ob.bufr')
    else:
        os.system(f'ln -sf {gfs_prebufr} ob.bufr')
#ahi.info 
...
os.system(f'ln -sf {WRFDA_DIR}/var/run/radiance_info/himawari-8-ahi.info_rttov {WRFDA_DIR}/var/run/radiance_info/himawari-8-ahi.info')
        ...
        if not os.path.exists('wrfvar_output'):
            ...
            for i in range(len(lines)):
                ...
                if "crtm_coef_path='./rttov_coeffs'" in lines[i]:
                    ...
                    os.system(f'ln -sf {WRFDA_DIR}/var/run/radiance_info/himawari-8-ahi.info_crtm {WRFDA_DIR}/var/run/radiance_info/himawari-8-ahi.info')
                    if os.path.exists(f'{VARBC_DIR}/{hh}/VARBC.out_{rtm_type}'):
                        os.system(f'cp {VARBC_DIR}/{hh}/VARBC.out_{rtm_type} VARBC.in')
                    else:
                        os.system(f'ln -sf {WRFDA_DIR}/var/run/VARBC.in .')
...
with open('VARBC.out', 'w') as f:
    f.write(fixed_text)
os.system(f'cp VARBC.out {VARBC_DIR}/{hh}/VARBC.out_{rtm_type}')
