...
5.模型结构的选择，根据数据及模型的用途和性质确认。
        ...
        不同的激活函数：Relu、Gelu、Sigmod、tanh、Softmax、LeakyReLU
        不同的Norm：Batchnorm、LayerNorm、RMSNorm、ChanLayerNorm
        不同的权重初始化：

        缓解模糊的结构：GAN、扩散
6.损失函数的选择
L1损失、L2损失、感知损失、BCE、分类损失、SSIM损失、自适应损失、傅里叶损失、KL散度、重建损失、GAN损失
7.优化器的选择
  余弦退火 warmup
7.训练过程的可视化分析
